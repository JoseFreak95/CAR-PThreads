...
* por columnas, es decir, cada hilo se encargará de sumar los elementos de las columnas que
...
*	- Una variable que determina el número de filas
*	- Una variable que determina el índice de la columna de inicio que le corresponde al hilo
*	- Una variable que determina el índice de la columna de fin que le corresponde al hilo
...
void* sumaElementosMatriz(void* args){
    ...
    for (int j = deberes->columnaIni; j < deberes->columnaFin; j++)
	{
        for(int i = 0; i < deberes->filas; i++)
	    {
...
void loadBalancing(tarea * deberes, int index, Balanceador * balanceador, int * offsetCopy, bool needBalancing)
{
	...
	{				  	//Estrategia de balanceo para división no exacta
		if ((*offsetCopy) != 0)
		{
			deberes->columnaIni = balanceador->currentIni;      //Se establece el índice de inicio para el subvector "index"
			...
			deberes->columnaFin = balanceador->currentFin;      //Se establece el índice de fin para el subvector "index"
		...
		{
			deberes->columnaIni = balanceador->currentIni;      //Se actualiza el índice de fin del último paquete creado.
			balanceador->currentFin += (balanceador->pack);		//Esto se debe a que como no quedan elemento que repartir
			deberes->columnaFin = balanceador->currentFin;      //la longitud del subvector es igual al tamaño del paquete.
			balanceador->currentIni = balanceador->currentFin;
		}
	}
	else //Estrategia de balanceo para división exacta
	{
		deberes->columnaIni = index*balanceador->pack;
		deberes->columnaFin = index*balanceador->pack + balanceador->pack;
	}
}
...
int main(int argc, char* argv[]){
	...
	if(argc != 4){
		...
		balanceador.pack = columnas / nHilos;
		balanceador.offset = columnas % nHilos;
        ...
        //filas y comprobamos que existan elementos restantes o no para elegir la estrategia 
        //de reparto de columnas, inicializamos la variable del resultado y creamos el hilo
		for(int i = 0; i<nHilos; i++){
			deberes[i].matriz = matrix;
			deberes[i].filas = filas;
