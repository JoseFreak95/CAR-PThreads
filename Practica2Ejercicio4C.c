...
* por elementos, es decir, cada hilo se encargará de sumar un número X de elementos
* según le correspondan en el reparto
*/
...
int* crearMatrizAleatoria(int filas, int columnas){
	...
	int* matrix = malloc(sizeof(int)*elementos);
...
*	- Una variable que determina el número de elementos
*	- Una variable que determina el índice de inicio que le corresponde al hilo
*	- Una variable que determina el índice de fin que le corresponde al hilo
...
void* sumaElementosMatriz(void* args){
	...
	{
    	deberes->resultadoLocal += (deberes->matriz)[i];
	}
...
int main(int argc, char* argv[]){
	...
	if(argc != 4){
		...
		int elementos = filas * columnas;						//Calculamos el número de elementos de la matriz (longitud del vector)
		...
		balanceador.pack = elementos / nHilos;
		balanceador.offset = elementos % nHilos;
        ...
        //elementos y comprobamos que existan elementos restantes o no para elegir la estrategia 
        //de reparto de columnas, inicializamos la variable del resultado y creamos el hilo
		for(int i = 0; i<nHilos; i++){
			deberes[i].matriz = matrix;
			deberes[i].elementos = elementos;

			loadBalancing(&deberes[i], i, &balanceador, &offsetCopy, (balanceador.offset != 0)?true:false);
